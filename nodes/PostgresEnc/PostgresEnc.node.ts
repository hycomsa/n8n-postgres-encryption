...
import { pgInsertV2, pgQueryV2, pgUpdate, wrapData } from './PostgresEnc.node.functions';

export class PostgresEnc implements INodeType {
		...
		credentialTest: {
			async postgresConnectionTest(
				...
				try {
					...
					const isSslEnabled: boolean = !['disable', undefined].includes(credentials.ssl as string | undefined);
					config.sslmode = (credentials.ssl as string) || 'disable';
					if (isSslEnabled) {
						config.ssl = {
					      ca: credentials.caCertificate,
					      cert: credentials.clientCertificate,
					      key: credentials.clientPrivateKey,
					    }
					} else {
						config.ssl = false;
					}
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		const credentials = await this.getCredentials('postgresEnc');
